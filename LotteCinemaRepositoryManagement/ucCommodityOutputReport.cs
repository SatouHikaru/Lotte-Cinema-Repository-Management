...
{
    public partial class ucCommodityOutputReport : UserControl
    {
        CommodityOutputReportBLL corBLL = new CommodityOutputReportBLL();

        public ucCommodityOutputReport()
        {
            InitializeComponent();
        }

        private static ucCommodityOutputReport instance;
        public static ucCommodityOutputReport Instance
        {
            get
            {
                if (instance == null)
                    instance = new ucCommodityOutputReport();
                return instance;
            }
        }

        private void ucCommodityOutputReport_Load(object sender, EventArgs e)
        {
        ...
        double CalculateTotalAmount()
        {
            ...
            for (int i = 0; i < dgvReport.RowCount; i++)
                totalAmount += double.Parse(dgvReport["Giá xuất", i].Value.ToString()) * double.Parse(dgvReport["Tổng số lượng", i].Value.ToString());
            return totalAmount;
        }

        private void splBtnGet_Click(object sender, EventArgs e)
        {
            dgvReport.DataSource = corBLL.Get(dtpTimeStart.Value, dtpTimeEnd.Value);
        ...
        private void splBtnPrint_Click(object sender, EventArgs e)
        {
            ...
            {
                frmCommodityOutputReport cor = new frmCommodityOutputReport(dtpTimeStart.Value, dtpTimeEnd.Value, double.Parse(txtEdtTotalAmount.Text));
                cor.ShowDialog();
            }
        }
        ...
        private void splBtnExport_Click(object sender, EventArgs e)
        {
            if (dgvReport.Rows.Count > 0)
            {
                ...
                Export(dt, "Thống kê hàng xuất", "THỐNG KÊ HÀNG XUẤT TỪ NGÀY " + dtpTimeStart.Value.ToString("dd-MM-yyyy") +
